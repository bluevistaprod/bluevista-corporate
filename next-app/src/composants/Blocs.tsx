...
function SurTitre({ enfant, sombre }: { enfant: string; sombre?: boolean }) {
  ...
  return (
   ...
   l'interlettrage (0,16 contre 0,18 em). */
    <div className={`mb-5 flex items-center gap-4 ${TYPO.surTitre}`} style={{ color: c }}>
...
/* ⚠️ `publique` ne sert qu'aux LIENS posés dans le texte enrichi : sans lui,
   un lien vers une réalisation sort en `/apercu/realisations/…` sur le site
   public — une adresse qui n'existe pas, et que Google suivrait. */
export function Blocs({ blocs, projets, publique }: { blocs: Bloc[]; projets: React.ReactNode; publique?: boolean }) {
  return (
    <>
      {blocs.map((b, i) => {
        ...
        return (
          ...
          >
            <Apparait>
              <UnBloc bloc={b} sombre={sombre} filet={filet} index={rang} projets={projets} aparte={aparte} publique={publique} />
            </Apparait>
          </section>
        );
      })}
...
function UnBloc({
  bloc, sombre, filet, index, projets, aparte, publique,
}: { bloc: Bloc; sombre: boolean; filet: boolean; index: number; projets: React.ReactNode; aparte?: string; publique?: boolean }) {
        ...
        <div className={large}>
          ...
          <div className={`grid gap-20 ${media ? "lg:grid-cols-2" : ""} items-start`}>
            <div className={media && index % 2 ? "lg:order-last" : ""}>
              ...
              <TexteRiche blocs={b.paragraphes} className="text-[1.0625rem] leading-[1.75] opacity-82" sombre={sombre} publique={publique} />
              {aparte && (
          ...
          <div className="relative z-10 max-w-[44ch] text-white">
            ...
            {b.texte && (
              <div className="mx-auto mb-8 mt-5 [&_p]:mx-auto [&_p]:max-w-none">
                <TexteRiche blocs={b.texte} className="text-[1.0625rem] leading-[1.75] text-white/85" sombre publique={publique} />
              </div>
            )}
            {b.boutonLien && (
              <a
