...
import { BLEU_CLAIR, SOMBRE } from "./palette";
import { COMPETENCES, METIERS } from "./plan-du-site";
import { liens, type JeuDeLiens } from "../shared/liens";
...
/**
 * ⛔ LE MENU EST CALCULÉ, PAS ÉCRIT. Les adresses différaient entre l'aperçu et
 * le site public ; en dur, l'en-tête d'une page publique renvoyait vers
 * `/apercu/agence`, une adresse qui n'existe pas pour un visiteur — et que
 * Google aurait suivie.
 */
 ...
 * traduire.
 */
const versionsDe = (L: JeuDeLiens) => [
  { code: "FR", titre: "Français", href: L.accueil, prete: true },
  { code: "EN", titre: "English", href: "#", prete: false },
  { code: "ES", titre: "Español", href: "#", prete: false },
];

export function EnTete({ opaque = false, publique }: { opaque?: boolean; publique?: boolean }) {
  const L = liens(publique);
  const MENU = menuDe(L);
  const VERSIONS = versionsDe(L);
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-[1500px] items-center justify-between px-8 py-5">
        <a href={L.accueil} className="flex items-center" aria-label="Bluevista — accueil">
