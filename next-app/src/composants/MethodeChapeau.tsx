...
import { BLEU, BLEU_CLAIR, CLAIR, TYPO } from "./palette";
...
/* ⚠️ Les adresses sont CALCULÉES : en dur, ce composant renvoyait vers
   `/apercu/metier/…` depuis la page publique de l'agence. */
const metiersDe = (L: ReturnType<typeof liens>) => [
  { cle: "film" as const, nom: "Film & contenu", url: L.metier("film") },
  { cle: "evenementiel" as const, nom: "Événement", url: L.metier("evenement") },
  { cle: "immersion" as const, nom: "Immersion", url: L.metier("immersion") },
];

export function MethodeChapeau({ publique }: { publique?: boolean }) {
  const METIERS = metiersDe(liens(publique));
