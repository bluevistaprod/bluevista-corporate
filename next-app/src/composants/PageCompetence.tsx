/**
 * LE CORPS DES PAGES DE SAVOIR-FAIRE, PARTAGÉ PAR L'APERÇU ET LE SITE PUBLIC.
 *
 * ⛔⛔ POURQUOI IL A FALLU L'EXTRAIRE. Les neuf pages de savoir-faire portent
 * l'essentiel du référencement du site — 11 redirections de l'ancien site les
 * visent, dont deux qui pèsent 97 et 85 clics sur douze mois. Elles n'ont
 * pourtant JAMAIS existé en public : elles ne vivaient que sous
 * `/apercu/competence/…`, c'est-à-dire nulle part pour un visiteur.
 * 👉 Le gabarit était fini, la route manquait. Un travail terminé qu'on ne
 * peut pas atteindre ne compte pas.
 *
 * ⚠️ `publique` ne change PAS le rendu, seulement les ADRESSES des liens.
 * Le même composant sert les deux routes ; c'est la seule façon d'éviter que
 * la version publique se mette à diverger de celle que Giz relit.
 */
import { notFound } from "next/navigation";
import { EnTete } from "./EnTete";
import { PiedDePage } from "./PiedDePage";
import { COMPETENCES, METIERS, competencesDuMetier } from "./plan-du-site";
import { lirePage, lireRealisationsDuProduit, lireRealisationsParSlugs, enParagraphes, imageUrl } from "../lib/sanity";
import { OFFRES } from "./offres";
import { BLEU, BLEU_CLAIR, CLAIR, CLAIR_SOUTENU, NOIR, SOMBRE, TYPO } from "./palette";
import { LecteurVideo, BaliseVideo } from "./LecteurVideo";
import { TexteRiche } from "./TexteRiche";
import { Blocs } from "./Blocs";
import { liens } from "../shared/liens";

export async function CorpsCompetence({ slug, publique }: { slug: string; publique?: boolean }) {
  ...
  const grilleProjets = projets.length > 0 ? (
    <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {projets.map(pr => (
        <a key={pr.slug} href={L.realisation(pr.slug)}
    ...
    <main style={{ background: CLAIR, color: SOMBRE }}>
      ...
      <section className="relative flex min-h-[62vh] flex-col justify-end overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-[1500px] px-8 pb-16 pt-40 text-white">
          ...
          <nav className="mb-6 text-[14px] text-white/60">
            <a href={L.accueil} className="hover:text-white">
              Accueil
            </a>
            <span className="mx-2">·</span>
            <a href={L.metier(metier.slug)} className="hover:text-white">
              {metier.nom}
            </a>
          </nav>
      ...
      {enBlocs ? (
        <>
            ...
            <section className="pb-4 pt-16" style={{ background: CLAIR_SOUTENU }}>
              <div className="mx-auto max-w-[1500px] px-8">
                <div className={imageIntro ? "grid items-center gap-14 lg:grid-cols-[1fr_1fr]" : "mx-auto max-w-[820px]"}>
                  <TexteRiche blocs={page.texte} className="max-w-[62ch] text-[1.0625rem] leading-[1.75] opacity-82" publique={publique} />
                  {imageIntro ? (
                    <img
          ...
          <Blocs blocs={page.blocs!} projets={grilleProjets} publique={publique} />
        </>
      ) : (
        <>
                ...
                return (
                  ...
                  >
                    ...
                    <div className={media ? "" : "max-w-[820px]"}>
                      ...
                      <TexteRiche blocs={sec.blocs} className="text-[1.0625rem] leading-[1.75] opacity-80" publique={publique} />
                    </div>
                  </article>
                );
              })}
