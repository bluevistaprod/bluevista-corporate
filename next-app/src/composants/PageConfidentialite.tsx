...
import { BLEU, BLEU_CLAIR, CLAIR, NOIR, SOMBRE, TYPO } from "./palette";
...
export function CorpsConfidentialite({  publique }: {  publique?: boolean }) {
    ...
    <main style={{ background: CLAIR, color: SOMBRE }}>
      ...
      <div className="mx-auto max-w-[900px] px-8 py-16">
        <p className="text-[14px] opacity-50">
          {/* ⭐ LA DERNIÈRE MENTION « À COMPLÉTER » DE LA PAGE, POSÉE LE 20/08.
              Giz : « pose la date du 4 septembre ». C'est la date de mise en
              ligne décidée, donc la date à laquelle ce texte devient celui qui
              s'applique.
              ⚠️ SI LA BASCULE GLISSE, CETTE DATE MENT. Elle est écrite en dur
              exprès : une date calculée automatiquement changerait toute seule
              au premier redéploiement, et une politique de confidentialité qui
              se date d'aujourd'hui à chaque visite n'a plus aucune valeur de
              preuve. C'est donc une ligne à revoir à la main si le 4 bouge. */}
