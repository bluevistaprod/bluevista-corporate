...
import { MethodeEnCercle } from "./Methode";
import { BLEU, BLEU_CLAIR, CLAIR, CLAIR_SOUTENU, NOIR, SOMBRE, TYPO } from "./palette";
import { type Metier as CleMetier } from "./plan-du-site";
import { offresDuMetier } from "./offres";
import { lirePage, imageUrl } from "../lib/sanity";
import { TOUTES_REALISATIONS } from "./realisations";
...
export async function CorpsMetier({ metier, publique }: { metier: string; publique?: boolean }) {
    ...
    <main style={{ background: CLAIR, color: SOMBRE }}>
                  ...
                  {o.produits.map(pr => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {n > 0 ? (
                          <a
                            href={`${L.realisations}?produit=${pr.slug}`}
      ...
      <section style={{ background: CLAIR_SOUTENU }} className="py-24 text-center">
        <h2 className={`mx-auto max-w-3xl px-8 ${TYPO.titre}`}>
          ...
          <span style={{ color: BLEU }}>{c.surTitre.toLowerCase()}</span>
        </h2>
        <div className="mt-10 flex flex-wrap justify-center gap-4 px-8">
          ...
          <a href={L.accueil} className="rounded-md border border-black/20 px-9 py-4 text-[16px] font-semibold">
            Revenir à l’accueil
          </a>
        </div>
      </section>
